...
## [From electric to electronic]{#xtocid15698970}
...
The historic precursor to the modern electronics era was invented by Thomas Edison in 1880 while developing the electric incandescent lamp. Edison found that a small current passed from the heated lamp filament to a metal plate mounted inside the vacuum envelop. (Figure [below](#03375.png) (a)) Today this is known as the "Edison effect". Note that the battery is only necessary to heat the filament. Electrons would still flow if a non-electrical heat source was used.

[]{#03375.png}

![](03375.png)

_(a) Edison effect, (b) Fleming valve or vacuum diode, (c) DeForest audion triode vacuum tube amplifier._

By 1904 Marconi Wireless Company adviser John Flemming found that an externally applied current (plate battery) only passed in one direction from filament to plate (Figure [above](#03375.png) (b)), but not the reverse direction (not shown). This invention was the vacuum diode, used to convert alternating currents to DC. The addition of a third electrode by Lee DeForest (Figure [above](#03375.png) (c)) allowed a small signal to control the larger electron flow from filament to plate.
...
## [[Active versus passive devices]{#xtocid15698971}]

[]{#Active device} []{#Passive device} []{#Saturable reactor}
...
If multiple amplifiers are staged, their respective gains form an overall gain equal to the product (multiplication) of the individual gains. (Figure [below](#03169.png)) If a 1 V signal were applied to the input of the gain of 3 amplifier in Figure [below](#03169.png) a 3 V signal out of the first amplifier would be further amplified by a gain of 5 at the second stage yielding 15 V at the final output.

[]{#03169.png}

![](03169.png)

_The gain of a chain of cascaded amplifiers is the product of the individual gains._

## [[Decibels]{#xtocid15698974}]
...
If these figures represented _power_ gains, we could directly apply the unit of bels to the task of representing the gain of each amplifier, and of the system altogether. (Figure [below](#03170.png))

[]{#03170.png}

![](03170.png)

_Power gain in bels is additive: 0.477 B + 0.699 B = 1.176 B._
...
Recalculating for decibels rather than bels, we notice the same phenomenon. (Figure [below](#03171.png))

[]{#03171.png}

![](03171.png)

_Gain of amplifier stages in decibels is additive: 4.77 dB + 6.99 dB = 11.76 dB._

[]{#Logarithm} []{#Antilogarithm} []{#Slide rule}
...
> A~P(dB)~ = 10 log~10~(P~O~ / P~I~) = 10 log~10~ (10 /1) = 10 log~10~ (10) = 10 (1) = 10 dB

**Example:** Find the power gain ratio A~P(ratio)~ = (P~O~ / P~I~) for a 20 dB Power gain.

> A~P(dB)~ = 20 = 10 log~10~ A~P(ratio)~

> 20/10 = log~10~ A~P(ratio)~

> 10^20/10^ = 10^log~10~\ (A~P(ratio)~)^

> 100 = A~P(ratio)~ = (P~O~ / P~I~)
...
> A~(dB)~ = 20 log~10~(V~O~ / V~I~) = 20 log~10~ (1 /0.01) = 20 log~10~ (100) = 20 (2) = 40 dB

**Example:** Find the voltage gain ratio A~V(ratio)~ = (V~O~ / V~I~) for a 20 dB gain amplifier having a 50 Ω input and out impedance.

> A~V(dB)~ = 20 log~10~ A~V(ratio)~

> 20 = 20 log~10~ A~V(ratio)~

> 20/20 = log~10~ A~P(ratio)~

> 10^20/20^ = 10^log~10~\ (A~V(ratio)~)^

> 10 = A~V(ratio)~ = (V~O~ / V~I~)
...
## [[Absolute dB scales]{#xtocid15698975}]

[]{#dB, sound measurements} []{#Sound intensity measurement}
...
[]{#dB, absolute power measurements} []{#dBW} []{#dBk}
...
## [[Attenuators]{#xtocid15698976}]

[]{#Attenuator}

Attenuators are passive devices. It is convenient to discuss them along with decibels. Attenuators weaken or _attenuate_ the high level output of a signal generator, for example, to provide a lower level signal for something like the antenna input of a sensitive radio receiver. (Figure [below](#03376.png)) The attenuator could be built into the signal generator, or be a stand-alone device. It could provide a fixed or adjustable amount of attenuation. An attenuator section can also provide isolation between a source and a troublesome load.
...
Common configurations are the **T** and **Π** networks shown in Figure [above](#03377.png) Multiple attenuator sections may be cascaded when even weaker signals are needed as in Figure [below](#03385.png).

### [[Decibels]{#xtocid15698977}]
...
The table in Figure [below](#03378.png) lists resistor values for the **T** and **Π** attenuators to match a 50 Ω source/ load, as is the usual requirement in radio frequency work.
...
For example, substitute the 10 dB values from the 50 Ω attenuator table for **R~1~** and **R~2~** as shown in Figure [below](#03379.png).
...
This shows us that we see 50 Ω looking right into the example attenuator (Figure [below](#03379.png)) with a 50 Ω load.
...
The table in Figure [below](#03380.png) lists resistor values for the **Π** attenuator matching a 50 Ω source/ load at some common attenuation levels. The resistors corresponding to other attenuation levels may be calculated from the equations.
...
The **10 dB** corresponds to a voltage attenuation ratio of **K=3.16** in the next to last line of the above table. Transfer the resistor values in that line to the resistors on the schematic diagram in Figure [above](#03381.png).

### [[L-section attenuator]{#xtocid156989710}]

[]{#Attenuator, L}

The table in Figure [below](#03382.png) lists resistor values for the **L** attenuators to match a 50 Ω source/ load. The table in Figure [below](#03383.png) lists resistor values for an alternate form. Note that the resistor values are not the same.

[]{#03382.png}

![](03382.png)

_L-section attenuator table for 50 Ω source and load impedance._
...
_Alternate form L-section attenuator table for 50 Ω source and load impedance._

### [[Bridged T attenuator]{#xtocid156989711}]

[]{#Attenuator, bridged T}

The table in Figure [below](#03384.png) lists resistor values for the bridged **T** attenuators to match a 50 Ω source and load. The bridged-T attenuator is not often used. Why not?

[]{#03384.png}

![](03384.png)

_Formulas and abbreviated table for bridged-T attenuator section, Z = 50 Ω._

### [[Cascaded sections]{#xtocid156989712}]

Attenuator sections can be cascaded as in Figure [below](#03385.png) for more attenuation than may be available from a single section. For example two 10 db attenuators may be cascaded to provide 20 dB of attenuation, the dB values being additive. The voltage attenuation ratio **K** or **V~I~/V~O~** for a 10 dB attenuator section is 3.16. The voltage attenuation ratio for the two cascaded sections is the product of the two **K**s or 3.16x3.16=10 for the two cascaded sections.

[]{#03385.png}

![](03385.png)

_Cascaded attenuator sections: dB attenuation is additive._

Variable attenuation can be provided in discrete steps by a switched attenuator. The example Figure [below](#03386.png), shown in the 0 dB position, is capable of 0 through 7 dB of attenuation by additive switching of none, one or more sections.

[]{#03386.png}

![](03386.png)

_Switched attenuator: attenuation is variable in discrete steps._
...
### [[RF attenuators]{#xtocid156989713}]

[]{#Attenuator, coaxial} []{#Attenuator, rf}
...
_Coaxial T-attenuator for radio frequency work._

A coaxial T-section attenuator consisting of resistive rods and a resistive disk is shown in Figure [above](#03387.png). This construction is usable to a few gigahertz. The coaxial Π version would have one resistive rod between two resistive disks in the coaxial line as in Figure [below](#03388.png).

[]{#Attenuator, coaxial} []{#Attenuator, rf} []{#03388.png}

![](03388.png)

_Coaxial Π-attenuator for radio frequency work._
...
[![Previous](previous.jpg)](index.html) [![Contents](contents.jpg)](index.html) [![Next](next.jpg)](SEMI_2.html)
